...
static void set_stack_inf(stack* stk, int line, const char* func, const char* file, const char* name) {

    (line && func && file && file && name)ASSERTED;
    stk->info.file = file;
    stk->info.func = func;
    stk->info.name = name;
    stk->info.line = line;
...
void do_dump(stack* stk, const char* func, int line, const char* file, FILE* out, int err) {
    ...
    for(size_t i = stk->size; i < stk->capacity; i++) {
        ...
        fprintf( out, " \t[%lu] = %lf ", i, ((el_type*)((char*)stk->data + CANARY_SIZE))[i]);
...
static int stack_resize_down(stack* stk) {
    ...
    *(unsigned long long*)((char*)(stk->data) + CANARY_SIZE + stk->capacity*sizeof(el_type)) = 0;
    #endif
    tmp = realloc(stk->data, sizeof(el_type) * (stk -> capacity) / resize_up_const + 2 * CANARY_SIZE );
...
int _stack_ctor(stack* stk, size_t size, int line, const char* func, const char* file, const char* name) {

    int err = 0;

    #ifdef NDEBUG
    if ( ( err = func_verify(stk)) == STACK_NULL_PTR || stk->status == ALIVE){ 
        dump(stk, LOG, err);
        return err; 
    } 
    #endif
    ...
    #ifdef NDEBUG
    set_stack_inf(stk, line, func, file, name);
